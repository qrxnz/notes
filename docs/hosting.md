...
1. In Account Home, select **Workers & Pages** > **Create application** > **Pages** > **Connect to Git**.
...
1. Commit these changes by doing `npx quartz sync`. This should deploy your site to `<github-username>.github.io/<repository-name>`.
...
1. Under "Custom Domain", type your custom domain and click "Save".
1. This next step depends on whether you are using an apex domain (`example.com`) or a subdomain (`subdomain.example.com`).
...
1. Import the Git repository containing your Quartz project.
1. Give the project a name (lowercase characters and hyphens only)
1. Check that these configuration options are set:
...
1. Go to the [Domains - Dashboard](https://vercel.com/dashboard/domains) page in Vercel.
1. Connect the domain to Vercel
1. Press "Add" to connect a custom domain to Vercel.
1. Select your Quartz repository and press Continue.
1. Enter the domain you want to connect it to.
1. Follow the instructions to update your DNS records until you see "Valid Configuration"
...
1. Update the `baseUrl` in `quartz.config.js` if necessary.
1. Ensure your domain has been added to the [Domains - Dashboard](https://vercel.com/dashboard/domains) page in Vercel.
1. Go to the [Vercel Dashboard](https://vercel.com/dashboard) and select your Quartz project.
1. Go to the Settings tab and then click Domains in the sidebar
1. Enter your subdomain into the field and press Add

## Netlify

1. Log in to the [Netlify dashboard](https://app.netlify.com/) and click "Add new site".
1. Select your Git provider and repository containing your Quartz project.
1. Under "Build command", enter `npx quartz build`.
1. Under "Publish directory", enter `public`.
1. Press Deploy. Once it's live, you'll have a `*.netlify.app` URL to view the page.
1. To add a custom domain, check "Domain management" in the left sidebar, just like with Vercel.
